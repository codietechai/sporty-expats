...
export function resolveNotificationDestination(
  ...
  const postId = normalizeResourceId(data?.postId);
  const storyId = normalizeResourceId(data?.storyId);
  const eventId = normalizeResourceId(data?.eventId);

  if (postId) {
    return { screen: "Post", params: { postId } };
  }
  if (storyId) {
    return { screen: "Post", params: { postId: storyId } };
  }
  if (eventId) {
    return { screen: "EventInfo", params: { eventId } };
  }
  ...
  //      /events/:id or /sportyevents/:id (event notifications)
  ...
  if (actionUrl) {
    ...
    if (url && url !== "#" && url !== "/") {
      ...
      // Notification URLs generated by the web app use `/sportyevents/:id`.
      // Sparkstrand can append its `notification` tracking marker directly to
      // a Mongo id (e.g. `...c2aanotification`), so normalise it before using
      // the value in an API request.
      const eventMatch = url.match(/\/(?:sporty)?events?\/([^/?#]+)/i);
      ...
      const eventIdFromUrl = normalizeResourceId(eventMatch?.[1]);
      if (eventIdFromUrl) return { screen: "EventInfo", params: { eventId: eventIdFromUrl } };
...
/**
 * Convert a notification identifier to the resource id expected by the API.
 * The notification provider's web tracking suffix is only removed when it is
 * attached to a 24-character Mongo ObjectId, so ordinary IDs are untouched.
 */
function normalizeResourceId(value: unknown): string | null {
  ...
  try {
    ...
    // Keep the original value when a malformed URL encoding is received.
  }

  const trackedMongoId = id.match(/^([a-f\d]{24})notification$/i);
  return trackedMongoId?.[1] ?? (id || null);
}
