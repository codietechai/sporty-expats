...
`connectUser` runs in two steps: an HTTP GET to `/api/v2/chat/users/connect` to verify credentials and populate `client.user` with the full server `ChatUser` record, then a WebSocket connection. After the promise resolves, wait for `SocketEvent.CONNECT` or check `client.isConnected()` before calling `room.watch()`.
...
```ts
const result = await room.updateMetadata(data: Record<string, any>): Promise<Record<string, any>>
```
...
| Event | Payload | Notes |
|---|---|---|
| `MESSAGE_NEW` | `ChatMessage` | New message. On the sender's client, the optimistic entry is replaced via `MESSAGE_UPDATED` instead |
